...
15. **Reward programs**: Admin CRUD `GET|POST /admin/reward-programs`, `GET|PATCH|DELETE /admin/reward-programs/{id}` (patch recomputes stored achievers); offers `GET /admin/reward-offers?program_id=` etc. **`reward_program_achievements`** table: paid **`partner_commission_schedules`** in program window — direct (level 0) vs team (level ≥ 1) vs goal (`businessType` **DIRECT** / **TEAM** / null = combined); **ULTIMATE** = one period **FULL**; **MONTHLY** = one row per calendar month. Partner `GET /partner/reward-programs` includes **`progress`** and **`hasEligibleReward`**. Admin **`GET /admin/reward-achievements`** (`programId`, `programType`, `goalReachedOnly`, `partnerId`); **`POST /admin/reward-achievements/recompute/{program_id}`** rebuilds all partners. SQL: `supabase_reward_programs_tables.sql`, `supabase_reward_program_achievements.sql`.

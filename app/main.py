...
11. **Self profile**: `PATCH /participant/profile` and `PATCH /partner/profile` (participant or partner token); same rule — **phone** cannot be updated (omit it from JSON; sending unknown keys returns 422). **Participant partner search**: `GET /participant/partners/search` with exactly one query param **`name`**, **`partnerId`**, or **`phone`** — returns **`partnerId`**, **`name`**, **`phone`** (one match).
